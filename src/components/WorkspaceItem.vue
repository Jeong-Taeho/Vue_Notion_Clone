<template>
  <li>
    <div
      ...
      "
    >
      <span class="material-icons" :class="{ active: showChildren }" @click.stop="showChildren = !showChildren"
        >play_arrow</span
      >
    ...
    <div v-if="!hasChildren && showChildren" class="no-children" :style="{ paddingLeft: `${14 * depth + 22}px` }">
      하위 페이지가 없습니다.
    </div>
    <ul v-if="hasChildren && showChildren">
      <workspace-item v-for="ws in workspace.documents" :key="ws.id" :workspace="ws" :depth="depth + 1" />
    </ul>
  </li>
</template>
